...
class MQTTTests(unittest.TestCase):
    ...
    def test_main(self):
        with patch("paho.mqtt.client.Client") as mock_mqtt:
            with patch(
                "modbus4mqtt.modbus4mqtt.mqtt_interface.loop_forever"
            ) as mock_mainloop:
                with patch(
                    "modbus4mqtt.modbus_interface.modbus_interface"
                ) as mock_modbus:
    ...
    def test_connect(self):
        with patch("paho.mqtt.client.Client") as mock_mqtt:
            with patch("modbus4mqtt.modbus_interface.modbus_interface") as mock_modbus:
                mock_modbus().connect.side_effect = self.connect_success
                m = modbus4mqtt.mqtt_interface(
                    "kroopit",
                    1885,
                    "brengis",
                    "pranto",
                    "./tests/test_connect.yaml",
                    MQTT_TOPIC_PREFIX,
                )
                m.connect()
                ...
                mock_mqtt().subscribe.assert_called_with(
                    MQTT_TOPIC_PREFIX + "/subscribe"
                )
        ...
        with patch("paho.mqtt.client.Client") as mock_mqtt:
            with patch("modbus4mqtt.modbus_interface.modbus_interface") as mock_modbus:
                ...
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/pub_on_change_false", 85, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/pub_on_change_true", 86, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/pub_on_change_absent", 87, retain=False
                )
                ...
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/pub_on_change_false", 15, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/pub_on_change_true", 16, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/pub_on_change_absent", 17, retain=False
                )
                mock_mqtt().publish.reset_mock()
                ...
                mock_mqtt().publish.assert_no_call(
                    MQTT_TOPIC_PREFIX + "/pub_on_change_true", 16, retain=False
                )
                mock_mqtt().publish.assert_no_call(
                    MQTT_TOPIC_PREFIX + "/pub_on_change_absent", 17, retain=False
    ...
    def test_retain_flag(self):
        with patch("paho.mqtt.client.Client") as mock_mqtt:
            with patch("modbus4mqtt.modbus_interface.modbus_interface") as mock_modbus:
                ...
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/retain_on", 1, retain=True
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/retain_off", 2, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/retain_absent", 3, retain=False
                )
    ...
    def test_default_table(self):
        with patch("paho.mqtt.client.Client") as mock_mqtt:
            with patch("modbus4mqtt.modbus_interface.modbus_interface") as mock_modbus:
                ...
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/holding", 1, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/input", 3, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/default", 2, retain=False
                )
    ...
    def test_value_map(self):
        with patch("paho.mqtt.client.Client") as mock_mqtt:
            with patch("modbus4mqtt.modbus_interface.modbus_interface") as mock_modbus:
                ...
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/value_map_absent", 1, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/value_map_present", "b", retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/value_map_misinterpretation",
                    "on",
                    retain=False,
                )
                mock_mqtt().publish.reset_mock()
                ...
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/value_map_present", 3, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/value_map_misinterpretation",
                    "off",
            ...
            with patch("modbus4mqtt.modbus_interface.modbus_interface") as mock_modbus:
                ...
                with self.assertLogs() as mock_logger:
                    ...
                    mock_mqtt().subscribe.assert_any_call(
                        MQTT_TOPIC_PREFIX + "/no_value_map"
                    )
                    mock_mqtt().subscribe.assert_any_call(
                        MQTT_TOPIC_PREFIX + "/value_map"
                    )
                    mock_mqtt().publish.reset_mock()
                    ...
                    msg = MQTTMessage(
                        topic=bytes(MQTT_TOPIC_PREFIX + "/no_value_map", "utf-8")
                    )
                    msg.payload = b"a"
                    m._on_message(None, None, msg)
                    self.assertIn(
                        "Failed to convert register value for writing. Bad/missing value_map? Topic: no_value_map, Value: b'a'",
                    ...
                    self.assertEqual(self.modbus_tables["holding"][2], 2)
                    msg = MQTTMessage(
                        topic=bytes(MQTT_TOPIC_PREFIX + "/value_map", "utf-8")
                    )
                    msg.payload = b"a"
                    m._on_message(None, None, msg)
                    ...
                    m._on_message(None, None, msg)
                    self.assertIn(
                        "Value not in value_map. Topic: value_map, value:",
                        mock_logger.output[-1],
                    )
                    self.assertEqual(self.modbus_tables["holding"][2], 1)
                    ...
                    m._on_message(None, None, msg)
                    self.assertIn(
                        "Failed to decode MQTT payload as UTF-8. Can't compare it to the value_map for register",
                        mock_logger.output[-1],
                    )
                    self.assertEqual(self.modbus_tables["holding"][2], 1)
    ...
    def test_scale(self):
        with patch("paho.mqtt.client.Client") as mock_mqtt:
            with patch("modbus4mqtt.modbus_interface.modbus_interface") as mock_modbus:
                ...
                mock_mqtt().publish.assert_any_call(
                    "prefix/scale_down_no_value_map", 1, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    "prefix/scale_with_value_map", "b", retain=False
                )

                msg = MQTTMessage(
                    topic=bytes(
                        MQTT_TOPIC_PREFIX + "/scale_up_no_value_map_set", "utf-8"
                    )
                )
                msg.payload = b"6"
                m._on_message(None, None, msg)
                ...
                msg = MQTTMessage(
                    topic=bytes(
                        MQTT_TOPIC_PREFIX + "/scale_down_no_value_map_set", "utf-8"
                    )
                )
                msg.payload = b"1"
                m._on_message(None, None, msg)
                ...
                msg = MQTTMessage(
                    topic=bytes(
                        MQTT_TOPIC_PREFIX + "/scale_with_value_map_set", "utf-8"
                    )
                )
                msg.payload = b"b"
                m._on_message(None, None, msg)
    ...
    def test_mask(self):
        with patch("paho.mqtt.client.Client") as mock_mqtt:
            with patch("modbus4mqtt.modbus_interface.modbus_interface") as mock_modbus:
                ...
                mock_mqtt().publish.assert_any_call(
                    "prefix/mask_no_scale", 0xFE00, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    "prefix/no_mask_no_scale", 0xFEF0, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    "prefix/mask_and_scale", 0xFE, retain=False
                )

                msg = MQTTMessage(
                    topic=bytes(MQTT_TOPIC_PREFIX + "/mask_and_scale_set", "utf-8")
                )
                msg.payload = b"255"
                m._on_message(None, None, msg)
                ...
                msg = MQTTMessage(
                    topic=bytes(MQTT_TOPIC_PREFIX + "/mask_no_scale_set", "utf-8")
                )
                msg.payload = b"255"
                m._on_message(None, None, msg)
        ...
        with patch("paho.mqtt.client.Client") as mock_mqtt:
            with patch("modbus4mqtt.modbus_interface.modbus_interface") as mock_modbus:
                ...
                m = modbus4mqtt.mqtt_interface(
                    ...
                    "brengis",
                    "pranto",
                    "./tests/test_address_offset.yaml",
                    MQTT_TOPIC_PREFIX,
                )
                m.connect()
                ...
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/publish", 2, retain=False
                )
    ...
    def test_json_key(self):
        ...
        with patch("paho.mqtt.client.Client") as mock_mqtt:
            with patch("modbus4mqtt.modbus_interface.modbus_interface") as mock_modbus:
                ...
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/publish2", '{"A": 3}', retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/publish", '{"A": 1, "B": "off"}', retain=True
                )
    ...
    def test_type(self):
        ...
        with patch("paho.mqtt.client.Client") as mock_mqtt:
            with patch("modbus4mqtt.modbus_interface.modbus_interface") as mock_modbus:
                ...
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/publish_uint16_1", 0, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/publish_uint16_2", 32767, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/publish_uint16_3", 32768, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/publish_uint16_4", 65535, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/publish_int16_1", 0, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/publish_int16_2", 32767, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/publish_int16_3", -32768, retain=False
                )
                mock_mqtt().publish.assert_any_call(
                    MQTT_TOPIC_PREFIX + "/publish_int16_4", -1, retain=False
                )

                msg = MQTTMessage(
                    topic=bytes(MQTT_TOPIC_PREFIX + "/publish_int16_1_set", "utf-8")
                )
                msg.payload = b"-2"
                m._on_message(None, None, msg)
                ...
                msg = MQTTMessage(
                    topic=bytes(MQTT_TOPIC_PREFIX + "/publish_uint16_1_set", "utf-8")
                )
                msg.payload = b"65533"
                m._on_message(None, None, msg)
    ...
    def test_register_validation(self):
        valids = [
            [  # Different json_keys for same topic
        ...
        invalids = [
            [  # Duplicate json_key for a topic
                {"address": 13050, "json_key": "A", "pub_topic": "ems/EMS_MODE"},
                {"address": 13050, "json_key": "A", "pub_topic": "ems/EMS_MODE"},
            ],
        ...
        for invalid in invalids:
            ...
            if not fail:
                self.fail(
                    "Didn't throw an exception checking an invalid register configuration"
                )

    def assert_modbus_call(
        self, mock_modbus, word_order=modbus4mqtt.modbus_interface.WordOrder.HighLow
    ):
        ...
        with patch("paho.mqtt.client.Client") as mock_mqtt:
            with patch("modbus4mqtt.modbus_interface.modbus_interface") as mock_modbus:
                ...
                m = modbus4mqtt.mqtt_interface(
                    ...
                    "brengis",
                    "pranto",
                    "./tests/test_word_order_low_high.yaml",
                    MQTT_TOPIC_PREFIX,
                )
                m.connect()
                self.assert_modbus_call(
                    mock_modbus, modbus4mqtt.modbus_interface.WordOrder.LowHigh
                )
