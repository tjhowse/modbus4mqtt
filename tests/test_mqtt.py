...
class MQTTTests(unittest.TestCase):
    ...
    def setUp(self):
        self.modbus_tables = [{'input': {}, 'holding': {}},{'input': {}, 'holding': {}}]
        self.connect_attempts = 0

    def tearDown(self):
        pass

    def read_modbus_register(self, table, unit, address, type='uint16'):
        if address not in self.modbus_tables[unit][table]:
        ...
        for i in range(modbus4mqtt.modbus_interface.type_length(type)):
            data = self.modbus_tables[unit][table][address + i]
            value = data.to_bytes(2,'big') + value
        value = modbus4mqtt.modbus_interface._convert_from_bytes_to_type(value, type)
        return value

    def write_modbus_register(self, table, unit, address, value, mask=0xFFFF, type='uint16'):
        old_value = self.modbus_tables[unit][table][address]
        and_mask = (1<<16)-1-mask
        or_mask = value
        new_value = (old_value & and_mask) | (or_mask & (mask))
        self.modbus_tables[unit][table][address] = new_value
    ...
    def test_pub_on_change(self):
        with patch('paho.mqtt.client.Client') as mock_mqtt:
            with patch('modbus4mqtt.modbus_interface.modbus_interface') as mock_modbus:
                ...
                self.modbus_tables[1]['holding'][1] = 85
                self.modbus_tables[1]['holding'][2] = 86
                self.modbus_tables[1]['holding'][3] = 87
                m.poll()
                ...
                self.modbus_tables[1]['holding'][1] = 15
                self.modbus_tables[1]['holding'][2] = 16
                self.modbus_tables[1]['holding'][3] = 17
                m.poll()
    ...
    def test_retain_flag(self):
        with patch('paho.mqtt.client.Client') as mock_mqtt:
            with patch('modbus4mqtt.modbus_interface.modbus_interface') as mock_modbus:
                ...
                m.connect()
                self.modbus_tables[1]['holding'][1] = 1
                self.modbus_tables[1]['holding'][2] = 2
                self.modbus_tables[1]['holding'][3] = 3
                m.poll()
    ...
    def test_default_table(self):
        with patch('paho.mqtt.client.Client') as mock_mqtt:
            with patch('modbus4mqtt.modbus_interface.modbus_interface') as mock_modbus:
                ...
                self.modbus_tables[1]['holding'][1] = 1
                self.modbus_tables[1]['holding'][2] = 2
                self.modbus_tables[1]['input'][1] = 3
                m.poll()
    ...
    def test_value_map(self):
        with patch('paho.mqtt.client.Client') as mock_mqtt:
            with patch('modbus4mqtt.modbus_interface.modbus_interface') as mock_modbus:
                ...
                self.modbus_tables[1]['holding'][1] = 1
                self.modbus_tables[1]['holding'][2] = 2
                self.modbus_tables[1]['holding'][3] = 1
                m.poll()
                ...
                self.modbus_tables[1]['holding'][2] = 3
                self.modbus_tables[1]['holding'][3] = 2
                m.poll()
        ...
        with patch('paho.mqtt.client.Client') as mock_mqtt:
            with patch('modbus4mqtt.modbus_interface.modbus_interface') as mock_modbus:
                ...
                with self.assertLogs() as mock_logger:
                    ...
                    mock_modbus().add_monitor_register.assert_any_call('holding', 1, 1, 'uint16')
                    mock_modbus().add_monitor_register.assert_any_call('holding', 1, 2, 'uint16')
                    ...
                    self.assertEqual(self.modbus_tables[1]['holding'][2], 2)
                    ...
                    self.assertEqual(self.modbus_tables[1]['holding'][1], 1)
                    # Publish a raw value valid for this topic
                    msg = MQTTMessage(topic=bytes(MQTT_TOPIC_PREFIX+'/no_value_map', 'utf-8'))
                    msg.payload = b'3'
                    m._on_message(None, None, msg)
                    self.assertEqual(self.modbus_tables[1]['holding'][1], 3)
                    ...
                    msg.payload = b'a'
                    m._on_message(None, None, msg)
                    self.assertEqual(self.modbus_tables[1]['holding'][2], 1)
    ...
    def test_scale(self):
        with patch('paho.mqtt.client.Client') as mock_mqtt:
            with patch('modbus4mqtt.modbus_interface.modbus_interface') as mock_modbus:
                ...
                mock_modbus().add_monitor_register.assert_any_call('holding', 1, 1, 'uint16')
                mock_modbus().add_monitor_register.assert_any_call('holding', 1, 2, 'uint16')
                mock_modbus().add_monitor_register.assert_any_call('holding', 1, 3, 'uint16')
                ...
                msg.payload = b'b'
                m._on_message(None, None, msg)
                self.assertEqual(self.modbus_tables[1]['holding'][3], 3)
    ...
    def test_mask(self):
        with patch('paho.mqtt.client.Client') as mock_mqtt:
            with patch('modbus4mqtt.modbus_interface.modbus_interface') as mock_modbus:
                ...
                mock_modbus().get_value.side_effect = self.read_modbus_register
                mock_modbus().set_value.side_effect = self.write_modbus_register
                self.modbus_tables[1]['holding'][1] = 0xFEF0
                ...
                msg.payload = b'255'
                m._on_message(None, None, msg)
                self.assertEqual(self.modbus_tables[1]['holding'][1], 0xFFF0)
                ...
                msg.payload = b'255'
                m._on_message(None, None, msg)
                self.assertEqual(self.modbus_tables[1]['holding'][1], 0x00F0)

                # This should set the LSb of the MSB.
                msg = MQTTMessage(topic=bytes(MQTT_TOPIC_PREFIX+'/mask_no_scale_set', 'utf-8'))
                msg.payload = b'511'
                m._on_message(None, None, msg)
                self.assertEqual(self.modbus_tables[1]['holding'][1], 0x01F0)
    ...
    def test_address_offset(self):
        with patch('paho.mqtt.client.Client') as mock_mqtt:
            with patch('modbus4mqtt.modbus_interface.modbus_interface') as mock_modbus:
                ...
                self.modbus_tables[1]['holding'][0] = 0
                self.modbus_tables[1]['holding'][1] = 1
                self.modbus_tables[1]['holding'][2] = 2
                self.modbus_tables[1]['holding'][3] = 3
                m.poll()
    ...
    def test_type(self):
        ...
        with patch('paho.mqtt.client.Client') as mock_mqtt:
            with patch('modbus4mqtt.modbus_interface.modbus_interface') as mock_modbus:
                ...
                self.modbus_tables[1]['holding'][0] = 0
                self.modbus_tables[1]['holding'][1] = 32767
                self.modbus_tables[1]['holding'][2] = 32768
                self.modbus_tables[1]['holding'][3] = 65535
                m.poll()
                ...
                msg.payload = b'-2'
                m._on_message(None, None, msg)
                self.assertEqual(self.modbus_tables[1]['holding'][0], 65534)

                msg = MQTTMessage(topic=bytes(MQTT_TOPIC_PREFIX+'/publish_int16_1_set', 'utf-8'))
                msg.payload = b'2'
                m._on_message(None, None, msg)
                self.assertEqual(self.modbus_tables[1]['holding'][0], 2)

                msg = MQTTMessage(topic=bytes(MQTT_TOPIC_PREFIX+'/publish_uint16_1_set', 'utf-8'))
                msg.payload = b'65533'
                m._on_message(None, None, msg)
                self.assertEqual(self.modbus_tables[1]['holding'][0], 65533)
    ...
    def test_word_order_setting(self):
        with patch('paho.mqtt.client.Client') as mock_mqtt:
            with patch('modbus4mqtt.modbus_interface.modbus_interface') as mock_modbus:
                ...
                m = modbus4mqtt.mqtt_interface('kroopit', 1885, 'brengis', 'pranto', './tests/test_type.yaml', MQTT_TOPIC_PREFIX)
                m.connect()
                mock_modbus.assert_any_call('192.168.1.90', 502, 5, scan_batching=None, variant=None, word_order=modbus4mqtt.modbus_interface.WordOrder.HighLow, baudrate=19200, method='rtu')
        ...
        with patch('paho.mqtt.client.Client') as mock_mqtt:
            with patch('modbus4mqtt.modbus_interface.modbus_interface') as mock_modbus:
                ...
                m.connect()
                mock_modbus.assert_any_call('192.168.1.90', 502, 5, scan_batching=None, variant=None, word_order=modbus4mqtt.modbus_interface.WordOrder.LowHigh, baudrate=19200, method='rtu')
