...
class ModbusTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.input_registers = self.modbusRegister(
            registers=list(range(0, modbus_interface.DEFAULT_READ_BATCHING * 10))
        )
        self.holding_registers = self.modbusRegister(
            registers=list(range(0, modbus_interface.DEFAULT_READ_BATCHING * 10))
        )
    ...
    def read_input_registers(self, address, count, device_id):
        return self.modbusRegister(
            registers=self.input_registers.registers[address : address + count]
        )
    ...
    def perform_variant_test(self, mock_modbus, variant, expected_framer):
        ...
        mock_modbus.assert_called_with(
            host="1.1.1.1", port=111, framer=expected_framer, retries=3, timeout=1
        )

    def test_connection_variants(self):
        with patch("modbus4mqtt.modbus_interface.ModbusTcpClient") as mock_modbus:
            self.perform_variant_test(
                mock_modbus, None, modbus_interface.FramerType.SOCKET
            )
            self.perform_variant_test(
                mock_modbus, "tcp", modbus_interface.FramerType.SOCKET
            )
            self.perform_variant_test(
                mock_modbus, "rtu-over-tcp", modbus_interface.FramerType.RTU
            )
        with patch("modbus4mqtt.modbus_interface.ModbusUdpClient") as mock_modbus:
            self.perform_variant_test(
                mock_modbus, "udp", modbus_interface.FramerType.SOCKET
            )
            self.perform_variant_test(
                mock_modbus, "rtu-over-udp", modbus_interface.FramerType.RTU
            )

        m = modbus_interface.modbus_interface(
            ip="1.1.1.1", port=111, variant="notexisiting"
        )
        self.assertRaises(ValueError, m.connect)

        m = modbus_interface.modbus_interface(
            ip="1.1.1.1", port=111, variant="notexisiting-over-tcp"
        )
        self.assertRaises(ValueError, m.connect)
    ...
    def test_connect(self):
        with patch("modbus4mqtt.modbus_interface.ModbusTcpClient") as mock_modbus:
            ...
            m = modbus_interface.modbus_interface(
                ip="1.1.1.1", port=111, read_batching=10
            )
            m.connect()
            mock_modbus.assert_called_with(
                host="1.1.1.1",
                port=111,
                framer=modbus_interface.FramerType.SOCKET,
                retries=3,
                timeout=1,
            )
            ...
            mock_modbus().read_holding_registers.assert_any_call(
                address=5, count=1, device_id=1
            )
            mock_modbus().read_input_registers.assert_any_call(
                address=6, count=1, device_id=1
            )

            m.set_value("holding", 5, 7)
            m.poll()

            mock_modbus().write_register.assert_any_call(
                address=5, value=7, device_id=1
            )
            ...
            mock_modbus().read_holding_registers.assert_any_call(
                address=5, count=10, device_id=1
            )
            mock_modbus().read_input_registers.assert_any_call(
                address=6, count=10, device_id=1
            )
            ...
            mock_modbus().read_holding_registers.assert_any_call(
                address=15, count=1, device_id=1
            )
            mock_modbus().read_input_registers.assert_any_call(
                address=6, count=10, device_id=1
            )
            mock_modbus().read_input_registers.assert_any_call(
                address=16, count=1, device_id=1
            )
            ...
            mock_modbus().read_input_registers.assert_any_call(
                address=16, count=3, device_id=1
            )
    ...
    def test_exception_on_write(self):
        with patch("modbus4mqtt.modbus_interface.ModbusTcpClient") as mock_modbus:
            ...
            with self.assertLogs() as mock_logger:
                ...
                m.set_value("holding", 5, 7)
                self.assertIn(
                    "ERROR:root:Failed to write to modbus device: Modbus Error: Oh noooo!",
    ...
    def test_read_batching_bad_value(self):
        with patch("modbus4mqtt.modbus_interface.ModbusTcpClient") as mock_modbus:
            with self.assertLogs() as mock_logger:
                mock_modbus().connect.side_effect = self.connect_success
                mock_modbus().read_input_registers.side_effect = (
    ...
    def test_write_batching_bad_value(self):
        with patch("modbus4mqtt.modbus_interface.ModbusTcpClient") as mock_modbus:
            with self.assertLogs() as mock_logger:
                ...
                self.assertIn(
                    "Overriding write batching to 1 due to single write mode.",
                    mock_logger.output[-1],
                )
    ...
    def test_multi_byte_write_counts(self):
        with patch("modbus4mqtt.modbus_interface.ModbusTcpClient") as mock_modbus:
            ...
            mock_modbus().write_register.assert_any_call(
                address=1, value=65535, device_id=1
            )
            ...
            mock_modbus().write_register.assert_any_call(
                address=1, value=int.from_bytes(b"\x29\x1e", "big"), device_id=1
            )
            mock_modbus().write_register.assert_any_call(
                address=2, value=int.from_bytes(b"\xac\xa7", "big"), device_id=1
            )
            mock_modbus().reset_mock()
            ...
            mock_modbus().write_register.assert_any_call(
                address=1, value=int.from_bytes(b"\x4b\xd6", "big"), device_id=1
            )
            mock_modbus().write_register.assert_any_call(
                address=2, value=int.from_bytes(b"\x73\x09", "big"), device_id=1
            )
            mock_modbus().write_register.assert_any_call(
                address=3, value=int.from_bytes(b"\xbc\x93", "big"), device_id=1
            )
            mock_modbus().write_register.assert_any_call(
                address=4, value=int.from_bytes(b"\xe5\x87", "big"), device_id=1
            )
            mock_modbus().reset_mock()
    ...
    def test_multi_byte_write_counts_LowHigh_order(self):
        with patch("modbus4mqtt.modbus_interface.ModbusTcpClient") as mock_modbus:
            ...
            mock_modbus().write_register.assert_any_call(
                address=1, value=int.from_bytes(b"\xac\xa7", "big"), device_id=1
            )
            mock_modbus().write_register.assert_any_call(
                address=2, value=int.from_bytes(b"\x29\x1e", "big"), device_id=1
            )
            mock_modbus().reset_mock()
            ...
            mock_modbus().write_register.assert_any_call(
                address=1, value=int.from_bytes(b"\xe5\x87", "big"), device_id=1
            )
            mock_modbus().write_register.assert_any_call(
                address=2, value=int.from_bytes(b"\xbc\x93", "big"), device_id=1
            )
            mock_modbus().write_register.assert_any_call(
                address=3, value=int.from_bytes(b"\x73\x09", "big"), device_id=1
            )
            mock_modbus().write_register.assert_any_call(
                address=4, value=int.from_bytes(b"\x4b\xd6", "big"), device_id=1
            )
            mock_modbus().reset_mock()
    ...
    def perform_multi_byte_read_write_values_test(self, write_mode):
        with patch("modbus4mqtt.modbus_interface.ModbusTcpClient") as mock_modbus:
            ...
            m = modbus_interface.modbus_interface(
                "1.1.1.1", 111, read_batching=1, write_mode=write_mode
            )
            m.connect()
            mock_modbus.assert_called_with(
                host="1.1.1.1",
    ...
    def test_multi_byte_read_write_values(self):
        self.perform_multi_byte_read_write_values_test(
            modbus_interface.WriteMode.Single
        )
